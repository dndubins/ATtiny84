// tinyServo84.ino
// Control up to 8 servos from pins PA0 to PA7 using CTC mode of Timer1 (ATtiny84)
// Clock speed = 8MHz
...
// Note: I have a potentiometer wired as a voltage divider on pin A7. This would conflict with the sketch if 8 servos
// are attached.

#include <avr/io.h>
#include <avr/interrupt.h>

#define NSVO 7            // number of servos to control (up to 8)
...
#define SVO1 PA0          // starting pin in PORTA for servo 0 (change as needed). 
...
unsigned int servo_PWs[NSVO] = { 1500, 1500, 1500, 1500, 1500, 1500, 1500 };  // Pulse widths in microseconds (default to center position)
bool servo_attached[NSVO] = { 0, 0, 0, 0, 0, 0, 0 };                    // Servo attachment status
...
void attachServo(byte servo_num) {
  if (servo_num < NSVO) {
    ...
    DDRA |= (1 << (SVO1 + servo_num));  // Set servo pin to OUTPUT mode
  }
}
...
void detachServo(byte servo_num) {
  if (servo_num < NSVO) {
    ...
    PORTA &= ~(1 << (SVO1 + servo_num));  // Set servo pin low
    DDRA &= ~(1 << (SVO1 + servo_num));   // Set servo pin to INPUT mode (less chatter when not doing anything)
  }
}
...
// Change s0..s6 to match dimensions of NSVO
void moveTo(int s0, int s1, int s2, int s3, int s4, int s5, int s6, int wait) {  // routine for controlling all servos slowly, simultaneously.
  // wait=0: as fast as possible. do not use wait < 10 msec.
  // Change structure of moveTo based on # servos needed (add coordinates)
  int loc[NSVO] = { s0, s1, s2, s3, s4, s5, s6 };  // create array for loc’ns
...
ISR(TIM1_COMPA_vect) {  // This is the ISR that will turn off the pins at the correct widths
  ...
  for (byte i = 0; i < NSVO; i++) {
    ...
    if (servo_attached[i]) {       // only turn on pin if servo attached
      PORTA |= (1 << (SVO1 + i));  // set correct servo pin high
    }
  }
  ...
  while ((TCNT1 * 8) < (SVOMAXPULSE + 100)) {  // multiply TCNT1 by microseconds/step
    ...
    for (byte i = 0; i < NSVO; i++) {
      if (servo_attached[i] && (TCNT1 * 8) > servo_PWs[i]) {
        ...
        PORTA &= ~(1 << (SVO1 + i));  // Set correct servo pin low
      }
    }
  }
}
